...
target_nationality_index = vectorizer.nationality_vocab.lookup_token(target_nationality)

# 生成5个样本
sampled_indices = sample_from_model(
    model,
    vectorizer,
    nationalities=[target_nationality_index] * 5,
    temperature=0.7  # 控制随机性，可调整（0.1~1.5）
)

# 解码并打印结果
decoded_surnames = decode_samples(sampled_indices, vectorizer)
print(f"Generated {target_nationality} Surnames:")
for i, name in enumerate(decoded_surnames, 1):
    print(f"{i}. {name}")